...
export default function AllMaintenancePage() {
  ...
  // — carrega lista de veículos do usuário (desejados+garagem) —
  const loadVehicles = useCallback(async () => {
    ...
    // use mesma lógica do hook useVehicles("all")
  ...
  // — carrega manutenções filtrando pelos seus veículos —
  const loadRecords = useCallback(async () => {
    ...
    if (!error && data) {
      setRecords(data as MaintenanceWithVehicle[]);
    }
    setLoading(false);
  }, [router, vehicles]);

  // no mount, primeiro veículos, depois registros
  useEffect(() => {
    loadVehicles();
  }, [loadVehicles]);

  useEffect(() => {
    if (vehicles.length > 0) loadRecords();
  }, [vehicles, loadRecords]);
  ...
  // — handlers de update/delete/status —
  ...
  // — filtragem em memória para UI —
  const filtered = useMemo(() => {
    return records.filter((r) => {
      if (vehicleFilter && r.vehicle.id !== vehicleFilter)
        return false;
      if (statusFilter && r.status !== statusFilter) return false;
      if (typeFilter && r.maintenance_type !== typeFilter)
        return false;
      return true;
    });
        ...
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <div className="relative">
            ...
            >
              {vehicleFilter
                ? vehicles
                    .find((v) => v.id === vehicleFilter)
                    ?.brand.concat(" ", vehicles.find((v) => v.id === vehicleFilter)?.model!)
                : "Todos os Veículos"}
              <ChevronDown className="w-4 h-4 text-gray-500" />
            </button>
            {showVehicleMenu && (
              <ul className="absolute right-0 mt-1 w-56 bg-white border rounded-md shadow-lg z-10">
                <li
                  onClick={() => {
                    setVehicleFilter("");
          ...
          <div className="flex space-x-2 bg-gray-100 rounded-full p-1">
            {["", "A fazer", "Feito", "Cancelado"].map((st) => (
              <button
                key={st}
                onClick={() =>
                  setStatusFilter(st as "" | "A fazer" | "Feito" | "Cancelado")
                }
          ...
          <div className="relative">
            ...
            {showTypeMenu && (
              <ul className="absolute right-0 mt-1 w-48 bg-white border rounded-md shadow-lg z-10">
                ...
                {Array.from(
                  new Set(records.map((r) => r.maintenance_type))
                ).map((t) => (
                  <li
                    key={t}
                    onClick={() => {
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm text-gray-700">
                    <div>
                      <strong>Veículo:</strong> {r.vehicle.brand}{" "}
                      {r.vehicle.model}
                    </div>
                    <div>
                    ...
                    <div>
                      <strong>Data:</strong> {r.scheduled_date ?? "—"}
                    </div>
                  </div>
                  ...
                  {r.maintenance_parts.length > 0 && (
                    <div className="mt-2 text-sm text-gray-700">
                      Peças: {r.maintenance_parts.length} — R${" "}
